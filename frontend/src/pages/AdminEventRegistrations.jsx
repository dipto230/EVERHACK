...
const convertToXML = () => {
  ...
  return (
    <div className="p-10 bg-[#020617] min-h-screen text-white">
      ...
      <div className="mb-6">
        <button onClick={exportXML} className="px-6 py-3 bg-cyan-500 text-black rounded-xl">Download XML</button>
      </div>
            ...
            <tbody>
              {registrations.map(reg => (
                <tr key={reg.id} className="border-b border-gray-700">
                  ...
                  <td className="p-3">{reg.createdAt && reg.createdAt.toDate ? reg.createdAt.toDate().toLocaleString() : ""}</td>
                </tr>
              ))}
            </tbody>
          </table>
